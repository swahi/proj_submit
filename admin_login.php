...
echo '<h3>Admin Login</h3><br />';
echo '<hr width="30%" align="left">';

//first, check if the user is already signed in. If that is the case, there is no need to display this page
if(isset($_SESSION['signed_in']) && $_SESSION['admin_signed_in'] == true)
{
		...
		else
		{
			...
			$sql = "SELECT  *
					FROM
						admin
					WHERE
						ausername = '" . mysql_real_escape_string($_POST['user_name']) . "'
					AND
						apassword = '". mysql_real_escape_string($_POST['user_pass']) . " '";
			...
			else
			{
				...
				else
				{
					...
					while($row = mysql_fetch_assoc($result))
					{
						...
						$_SESSION['admin_user_name'] 	= $row['ausername'];
